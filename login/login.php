...
<body class="flex items-center justify-center flex-wrap text-center flex-col gap-[1.25rem] backgroundImg text-[whitesmoke] MV-boli">
    ...
    <form action="login_process.php" method="post" id="loginForm" class="flex items-center justify-center flex-col bg-slate-700 opacity-95 MV-boli p-4 border-4 border-double border-purple-800 rounded-bl-[100px] rounded-tr-[100px] w-60 sm:min-w-72 sm:p-2 gap-8">
        ...
        <input type="text" name="username" id="username" required placeholder="Ex : Paladin67" class="transition duration-500 hover:scale-110 hover:border-b-4 hover:shadow-xl hover:shadow-black border-2 border-solid border-black border-b-2  border-b-purple-600 text-center bg-gray-800 text-white opacity-75 rounded-tl-md rounded-br-md h-8 w-11/12">

        <label for="password">Enter your password :</label>
        <div class="w-11/12 flex flex-row flex-nowrap p-0">
            <input type="password" required name="password" id="password" placeholder="*********" class="transition duration-500 hover:scale-110 hover:border-b-4 hover:shadow-xl hover:shadow-black border-2 border-solid border-black border-b-2  border-b-purple-600 text-center bg-gray-800 text-white opacity-75 rounded-tl-md rounded-br-md h-8 w-full flex-shrink-0 box-border">
            <img src="../assets/eye/eye-crossed.png" id="eyeSlashed" alt="Eye slashed" title="Display the password" class="h-[1.25rem] w-[1.25rem] select-none relative top-1 right-6 hover:scale-125">
        </div>        
        
        <input type="submit" value="Submit" id="submit" name="submit" class="cursor-pointer hover:border-purple-900 border-2 border-solid border-white rounded-2xl p-2 hover:bg-purple-200 hover:text-black transition duration-200 hover:scale-110 min-w-32 hover:shadow-sm hover:shadow-white active:bg-purple-400 active:font-thin">
